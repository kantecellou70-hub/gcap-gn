...
export async function runHealthCheck(): Promise<HealthReport> {
  const [supabaseResult, authResult, databaseResult] = await Promise.all([
    checkSupabase(),
    checkAuth(),
    checkDatabase(),
  ])

  const allOk = [supabaseResult, authResult, databaseResult].every(
    (c) => c.status === 'ok'
  )
  const anyError = [supabaseResult, authResult, databaseResult].some(
    (c) => c.status === 'error'
  )
